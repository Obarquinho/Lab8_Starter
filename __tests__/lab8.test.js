describe('Basic user flow for Website', () => {
  ...
  beforeAll(async () => {
    await page.goto('https://obarquinho.github.io/Lab8_Starter/');
  });
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const inner = await but.getProperty('innerText');
    const vale = await inner.jsonValue();
    expect(vale).toBe("Remove from Cart");
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const inner = await cartCnt.getProperty('innerText');
    const vale = await inner.jsonValue();
    expect(vale).toBe("20");
  }, 10000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    // const vale = localStorage.getItem('cart');
